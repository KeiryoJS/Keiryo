...
import type { APIExtendedInvite, APIInvite, InviteTargetUserType } from "discord-api-types";
...
export class Invite extends Resource {
  ...
  protected _patch(data: APIInvite | APIExtendedInvite): this {
    ...
    this.inviter = data.inviter
      ? this.client.users.cache.get(data.inviter.id) ?? null
      : null;

    this.targetUser = data.target_user
      ? this.client.users.cache.get(data.target_user.id) ?? null
      : null;
